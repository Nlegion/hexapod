...
// Оптимизированные траектории для широкой и плавной походки
// Формат: [шаг][COXA, FEMUR, TIBIA] - все значения в пределах MIN_PULSE(1000) - MAX_PULSE(2000)
constexpr int TRANSFER_TRAJ[4][3] = {
  // Фаза ПЕРЕНОСА: увеличенная высота подъема для более уверенного клиренса
  {1450, 1700, 1250}, // Шаг 0: Высокий подъем + широкое движение вперед (FEMUR: 1620→1700, TIBIA: 1320→1250)
  {1550, 1750, 1200}, // Шаг 1: МАКСИМАЛЬНЫЙ подъем + максимальное движение вперед (FEMUR: 1680→1750, TIBIA: 1280→1200)
  {1540, 1650, 1320}, // Шаг 2: Контролируемое опускание с высоким клиренсом (FEMUR: 1580→1650, TIBIA: 1380→1320)
  {1500, 1500, 1500}  // Шаг 3: Нейтральная позиция (полностью опущены)
};

constexpr int SUPPORT_TRAJ[4][3] = {
  // Фаза ОПОРЫ: оптимизированная траектория для широкого и стабильного толчка
  {1540, 1480, 1520}, // Шаг 0: Широкая начальная позиция с мощной опорой
  {1520, 1460, 1540}, // Шаг 1: Активное отталкивание с градуальным движением назад
  {1480, 1440, 1560}, // Шаг 2: Продолжение мощного толчка назад
  {1420, 1420, 1580}  // Шаг 3: Максимальное широкое отталкивание для эффективного движения
};
