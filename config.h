...
//
// КЛЮЧЕВОЕ ПОНИМАНИЕ: 
// - TRANSFER (нога в воздухе): COXA движется от НАЗАД (-) к ВПЕРЁД (+)
// - SUPPORT (нога на земле): COXA движется от ВПЕРЁД (+) к НАЗАД (-), толкая тело вперёд
//
constexpr int TRANSFER_TRAJ[4][3] = {
  // Фаза ПЕРЕНОСА: нога поднимается и выносится вперёд
  {1400, 1680, 1280}, // Шаг 0: Старт из задней позиции + начало подъёма (COXA=-100 НАЗАД)
  {1500, 1750, 1200}, // Шаг 1: Нейтраль + максимальный подъём (COXA=0, высоко)
  {1600, 1750, 1200}, // Шаг 2: Вперёд + остаёмся высоко (COXA=+100 ВПЕРЁД!)
  {1600, 1500, 1500}  // Шаг 3: Опускаемся в передней позиции (COXA=+100, на земле)
};

constexpr int SUPPORT_TRAJ[4][3] = {
  // Фаза ОПОРЫ: нога на земле толкает назад, двигая тело вперёд
  {1600, 1500, 1500}, // Шаг 0: Начало из передней позиции (COXA=+100 ВПЕРЁД)
  {1550, 1480, 1520}, // Шаг 1: Движение назад + опора (COXA=+50)
  {1480, 1460, 1540}, // Шаг 2: Продолжаем толкать назад (COXA=-20)
  {1400, 1450, 1550}  // Шаг 3: Максимально назад + мощный толчок (COXA=-100 НАЗАД!)
};
